  ...
  module IssueExtension
    ...
    included do
      ...
      def self.find_by_serialnumber(serial)
        find_by_custom_field_value(serial, ::RLM::Setup::IssueCustomFields.serialnumber.id, exact: true)
      end
      ...
      def self.find_by_custom_field_value(value, cf_id, exact: false)
        result = self.joins("LEFT JOIN custom_values on customized_id = issues.id")
          .where("custom_values.customized_type = 'Issue' AND custom_values.custom_field_id = #{cf_id}")
          if exact
            result.where("lower(custom_values.value) = '#{value.to_s.downcase}'")
          else
            result.where("lower(custom_values.value) LIKE '%#{value.to_s.downcase}%'")
          end
      end
